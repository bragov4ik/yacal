...
func TestElements(t *testing.T) {
	tests := []testCase{
		...
		parsesTo("(setq x true)", list(atom("setq"), atom("x"), true)),
		parsesTo("(setq x false)", list(atom("setq"), atom("x"), false)),
		parsesTo("(car '(1 2 3))", list(atom("car"), list(atom("quote"), list(1, 2, 3)))),
