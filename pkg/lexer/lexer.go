...
func isAtomEnd(r rune) bool {
	return unicode.IsSpace(r) || r == lex.EOF || r == ')'
}
...
func readIdent(l *lex.State) lex.StateFn {
	...
	default:
		id := tok.NewIdent(ident)
		l.Emit(l.TokenPos(), tok.IDENT, id)
	}
	return nil
}
...
func readChar(l *lex.State) lex.StateFn {
	...
	if got, assert := assertChar(l, '\''); assert {
		l.Errorf(l.TokenPos(), "Character symbol should end with single quote, not with `%v'", string(got))
	}
